...
fetch("data/movies.json")
  ...
  .then(data => {
    ...
    // ===============================
    // ▶ CREATE PLAYERS + DOWNLOAD BUTTONS (PREMIUM CHECK)
    // ===============================
    let playersHTML = "";
    if (movie.players && movie.players.length > 0) {
      movie.players.forEach(player => {
        playersHTML += `<button class="btn btn-player" onclick='handlePremiumPlayer(${JSON.stringify(movie)}, "${player.link}")'>${player.name}</button>`;
      });
...
function downloadMovie(link) {
  const downloadLink = link.replace("/preview", "/view");
