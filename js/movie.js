...
// 🔐 YOUTUBE API KEY
// ===============================
const API_KEY = "AIzaSyBPPhDhVem-bSzZjGIlysFfnAax9bKQ2aM";

// ===============================
// 💰 COINS SYSTEM + DOWNLOAD LOCK
// ===============================
const DAILY_COINS = 5;
...
function setCoins(amount){ localStorage.setItem(COINS_KEY, amount); updateCoinsUI(); }
...
function claimDailyCoins(){
    ...
    } else { alert("⏳ Already claimed today. Wait for countdown."); }
}
...
function setDownloadLock(movieId, hours = 6){
    const key = "cinemax_download_" + movieId;
    localStorage.setItem(key, Date.now() + hours*60*60*1000);
}
...
function updateDownloadCountdown(movieId){
    ...
    const interval = setInterval(()=>{
        ...
        if(remain <= 0){ el.innerText=""; clearInterval(interval); return; }
...
function attemptDownload(link,movieId){
    if(getCoins() <= 0){ alert("❌ Not enough coins. Claim daily coins first!"); return; }
    if(!canDownload(movieId)){ alert("⏳ Download locked for 6 hours."); return; }
    setCoins(getCoins()-1);
...
.then(data=>{
    ...
    if(movie.players && movie.players.length>0){
        movie.players.forEach(p=>{ playersHTML += `<button class="btn btn-player" onclick="goAdPage('${p.link}')">${p.name}</button>`; });
    ...
    let socialHTML = `
        <div style="margin-top:20px; display:flex; gap:12px;">
          <a href="https://www.facebook.com/sharer/sharer.php?u=${currentURL}" target="_blank"><img src="https://img.icons8.com/color/48/000000/facebook-new.png" width="35" title="Share on Facebook"></a>
          <a href="https://wa.me/?text=${shareURL}" target="_blank"><img src="https://img.icons8.com/color/48/000000/whatsapp.png" width="35" title="Share on WhatsApp"></a>
          <a href="https://twitter.com/intent/tweet?url=${currentURL}&text=Watch ${encodeURIComponent(movie.title)}" target="_blank"><img src="https://img.icons8.com/color/48/000000/twitter--v1.png" width="35" title="Share on Twitter"></a>
        </div>
    `;
    ...
    getTrailer(movie.title).then(trailerId=>{
            ...
            <div style="max-width:1000px;margin:auto;padding:20px;color:white;font-family:Poppins,sans-serif;">
                ...
                <div style="display:flex;gap:25px;margin-top:30px;flex-wrap:wrap;">
                    <div style="flex:1;min-width:250px;">
                        ...
                        <div style="margin-top:10px;font-size:1.2em;"><strong>IMDb:</strong> <span style="color:#ffcc00;">${getStars(movie.imdb)}</span> <span style="color:#aaa;">(${movie.imdb}/10)</span></div>
                    </div>
        ...
        form.addEventListener("submit", e=>{
            ...
            .then(res=>res.json()).then(()=>{successMsg.style.display="block";form.reset();submitBtn.style.display="block";});
        });
...
function downloadMovie(link){ window.open(link.replace("/preview","/view"),"_blank"); }
function getStars(r){ r=parseFloat(r);let f=Math.floor(r/2);let h=(r%2)>=1?true:false;let e=5-f-(h?1:0);return"⭐".repeat(f)+(h?"✨":"")+"☆".repeat(e); }
function getTrailer(name){ let q=encodeURIComponent(name+" trailer"); let url=`https://youtube.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=1&q=${q}&key=${API_KEY}`; return fetch(url).then(r=>r.json()).then(d=>d.items&&d.items.length>0?d.items[0].id.videoId:"").catch(()=> ""); }
function goAdPage(link){ let p=new URLSearchParams(window.location.search); window.location.href="adpage.html?id="+p.get("id")+"&play="+encodeURIComponent(link); }
...
setInterval(()=>{
    ...
    const el=document.getElementById("coinCountdown"); if(!el) return;
